...
describe('GetUserUseCase', () => {
  ...
  describe('executeByEmail', () => {
    ...
    it('should throw error if email format is invalid', async () => {
      await expect(getUserUseCase.executeByEmail('invalid-email')).rejects.toThrow(
    ...
    it('should throw error if user not found by email', async () => {
      ...
      await expect(getUserUseCase.executeByEmail('notfound@example.com')).rejects.toThrow(
        'User not found'
      );

      expect(mockUserRepository.getUserByEmail).toHaveBeenCalledWith('notfound@example.com');
    });
  });
});
